<template>
    <!-- Add Record modal -->
	<div id="add-record" class="hs-overlay hs-overlay-open:opacity-100 hs-overlay-open:duration-300 hidden size-full fixed top-0 start-0 z-80 opacity-0 overflow-x-hidden transition-all overflow-y-auto pointer-events-none" role="dialog" tabindex="-1">
		<div class="min-h-screen flex items-center justify-center px-4">
			<div class="max-w-[420px] w-full mx-auto">
				<div class="bg-white rounded-md pointer-events-auto p-5">
					<div class="flex items-center justify-between mb-5">
						<h2 class="text-base font-bold text-gray-900 mb-0">Add Record</h2>
						<button type="button" class="size-8 inline-flex justify-center items-center rounded-md border border-border-color bg-white text-gray-900 hover:bg-light cursor-pointer" data-hs-overlay="#add-record" aria-label="Close">
							<i class="ph ph-x"></i>
						</button>
					</div>
					<form @submit.prevent="submitForm" class="space-y-4">
						<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
							<div>
								<label class="text-sm font-semibold text-gray-900 mb-1 inline-block">ID <span class="text-danger">*</span></label>
								<input type="text" value="WFL0001" class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-gray-50 focus:outline-none focus:ring-0 text-gray-600 dark:bg-gray-100" readonly>
							</div>
							<div>
								<label class="text-sm font-semibold text-gray-900 mb-1 inline-block">Module <span class="text-danger">*</span></label>
						...
						<div>
							<label class="text-sm font-semibold text-gray-900 mb-1 inline-block">Name <span class="text-danger">*</span></label>
							<input type="text" class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
						</div>
						<div>
						...
						<div class="flex items-center justify-between gap-3 mt-5">
							<button type="button" class="btn-sm bg-white border border-border-color text-gray-900 hover:bg-light cursor-pointer" data-hs-overlay="#add-record">Cancel</button>
	...
	<div id="edit-modal" class="hs-overlay hs-overlay-open:opacity-100 hs-overlay-open:duration-300 hidden size-full fixed top-0 start-0 z-80 opacity-0 overflow-x-hidden transition-all overflow-y-auto pointer-events-none" role="dialog" tabindex="-1">
		<div class="min-h-screen flex items-center justify-center px-4">
			<div class="max-w-[420px] w-full mx-auto">
				<div class="bg-white rounded-md pointer-events-auto p-5">
					<div class="flex items-center justify-between mb-5">
						<h2 class="text-base font-bold text-gray-900 mb-0">Edit Record</h2>
					...
					<form @submit.prevent="submitForm" class="space-y-4">
						<div class="grid grid-cols-2 sm:grid-cols-2 gap-3">
							...
							<div>
								<label class="text-sm font-semibold text-gray-900 mb-1 inline-block">Module <span class="text-danger">*</span></label>
								<input type="text" value="Purchase" class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
							</div>
						</div>
						<div>
							<label class="text-sm font-semibold text-gray-900 mb-1 inline-block">Name <span class="text-danger">*</span></label>
							<input type="text" value="Purchase Order Approval" class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
						</div>
						<div>
							<label class="text-sm font-semibold text-gray-900 mb-1 inline-block">Status <span class="text-danger">*</span></label>
							<select class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">
						...
						<div>
							<label class="text-sm font-semibold text-gray-900 mb-1 inline-block">Description</label>
							<textarea rows="4" class="w-full px-3 py-2 text-sm border border-border-color rounded-md bg-white focus:outline-none focus:ring-0">PO approvals by value</textarea>
						</div>
						<div class="flex items-center justify-between gap-3 mt-5">
...
export default {
    setup() {
        const submitForm = () => {
            router.push("/applications/workflow-approvals");
        };
