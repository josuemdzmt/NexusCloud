  ...
  <Form :validation-schema="validationSchema" :initial-values="initialValues" @submit="handleSubmit" v-slot="{ errors, isSubmitting, setValues, values }" ref="formRef">
    ...
    <div v-if="variant === 'record'" class="grid grid-cols-12 gap-3">
      <div class="col-span-12 lg:col-span-8 space-y-3">
        <slot name="main" :errors="errors" :isSubmitting="isSubmitting" :setValues="setValues" :values="values"></slot>
      </div>
      <div v-if="$slots.sidebar" class="col-span-12 lg:col-span-4 space-y-3">
        <slot name="sidebar" :errors="errors" :isSubmitting="isSubmitting" :setValues="setValues" :values="values"></slot>
      </div>
    </div>
    ...
    <div v-else-if="variant === 'document'" class="bg-white border border-border-color rounded-md p-5 shadow-sm">
      <slot name="document" :errors="errors" :isSubmitting="isSubmitting" :setValues="setValues" :values="values">
    ...
    <slot name="footer" :errors="errors" :isSubmitting="isSubmitting" :setValues="setValues" :values="values">
